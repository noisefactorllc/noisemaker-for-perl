...
sub emit {
    ...
    push @L, '    $' . $main->{mangled} . '->();';
    my $out_name =
        $self->{varyings}{ $self->{outputs}[0] }
        ? '$ctx->{uv}'
        : '$g->{' . p_ident($self->{outputs}[0]) . '}';
    push @L, "    my \$_c = $out_name;";
    push @L,
        '    $out->[0] = $rt->f32($_c->[0]); $out->[1] = $rt->f32($_c->[1]); '
        . '$out->[2] = $rt->f32($_c->[2]); $out->[3] = $rt->f32($_c->[3]);';
    push @L, '};';
...
sub _e_call {
    ...
    if ($self->{overloads}{$name}) {
        ...
        if (@{ $fn->{out_idxs} || [] }) {    # out/inout: unpack outputs
            ...
            return (
                '($_retc, ' . join(', ', @targets) . ") = \$$fn->{mangled}->(" . join(', ', @codes) . ')',
                $fn->{ret}
            );
        }
