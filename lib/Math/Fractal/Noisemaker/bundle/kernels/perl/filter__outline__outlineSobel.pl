# Generated by Math::Fractal::Noisemaker::Transpiler - do not edit.
use strict;
use warnings;
no warnings qw(uninitialized);
my $run_pixel = sub {
    my ($ctx, $out) = @_;
    my $rt = $ctx->rt;
    my $U = $ctx->uniforms;
    my $g = {};
    my ($wrapCoord__int_int, $distanceMetric__float_float_int, $main__void);
    my $_retc;
    my $_u_tileOffset = exists $U->{'tileOffset'} ? $U->{'tileOffset'} : $rt->construct(2, 0.0);
    my $_u_fullResolution = exists $U->{'fullResolution'} ? $U->{'fullResolution'} : $rt->construct(2, 0.0);
    my $_u_valueTexture = $ctx->texture_binding('valueTexture');
    my $_u_sobelMetric = exists $U->{'sobelMetric'} ? $U->{'sobelMetric'} : $rt->f(0.0);
    my $_u_thickness = exists $U->{'thickness'} ? $U->{'thickness'} : $rt->f(0.0);
    my $_u_renderScale = exists $U->{'renderScale'} ? $U->{'renderScale'} : $rt->f(0.0);
    $g->{fragColor} = $rt->construct(4, 0.0);
    $wrapCoord__int_int = sub {
        my ($value, $size) = @_;
        my ($wrapped);
        if ($rt->binary('<=', $size, $rt->i(0))) {
            return $rt->i(0);
        }
        $wrapped = $rt->binary('%', $value, $size, 1, 'int');
        if ($rt->binary('<', $wrapped, $rt->i(0))) {
            $wrapped = $rt->binary('+', $wrapped, $size, 1, 'int');
        }
        return $wrapped;
    };
    $distanceMetric__float_float_int = sub {
        my ($gx, $gy, $metric) = @_;
        my ($abs_gx, $abs_gy, $cross);
        $abs_gx = $rt->component_wise('abs', $gx);
        $abs_gy = $rt->component_wise('abs', $gy);
        $cross = $rt->f(0.0);
        if ($rt->binary('==', $metric, $rt->i(2))) {
            return $rt->binary('+', $abs_gx, $abs_gy, 1, 'float');
        } else {
            if ($rt->binary('==', $metric, $rt->i(3))) {
                return $rt->component_wise('max', $abs_gx, $abs_gy);
            } else {
                if ($rt->binary('==', $metric, $rt->i(4))) {
                    $cross = $rt->binary('/', $rt->binary('+', $abs_gx, $abs_gy, 1, 'float'), $rt->f(1.4139999999999999), 1, 'float');
                    return $rt->component_wise('max', $cross, $rt->component_wise('max', $abs_gx, $abs_gy));
                } else {
                    return $rt->component_wise('sqrt', $rt->binary('+', $rt->binary('*', $gx, $gx, 1, 'float'), $rt->binary('*', $gy, $gy, 1, 'float'), 1, 'float'));
                }
            }
        }
    };
    $main__void = sub {
        my ($_for0_first, $_for1_first, $coord, $dimensions, $globalCoord, $gx, $gy, $idx, $kx, $ky, $magnitude, $metric, $normalized, $offset, $sampleX, $sampleY, $samples);
        $globalCoord = $rt->binary('+', $rt->swizzle($ctx->{frag_coord}, 'xy'), $_u_tileOffset, 2, 'float');
        $dimensions = $rt->texture_size($_u_valueTexture);
        if ((($rt->binary('==', $rt->swizzle($dimensions, 'x'), $rt->i(0))) || ($rt->binary('==', $rt->swizzle($dimensions, 'y'), $rt->i(0))) ? 1 : 0)) {
            @{$g->{fragColor}} = map { $rt->f32($_) } @{($rt->construct(4, $rt->f(0)))};
            return;
        }
        $coord = $rt->construct(2, $rt->swizzle($ctx->{frag_coord}, 'xy'), 'int');
        $metric = $rt->construct(1, $_u_sobelMetric, 'int');
        $offset = $rt->component_wise('max', $rt->i(1), $rt->construct(1, $rt->binary('*', $_u_thickness, $_u_renderScale, 1, 'float'), 'int'));
        $samples = $rt->new_array($rt->i(9), 1);
        $idx = $rt->i(0);
        $ky = $rt->unary('-', $rt->i(1));
        $_for0_first = 1;
        for my $_for0 (0 .. 1048575) {
            if (!$_for0_first) {
                $ky = $rt->binary('+', $ky, $rt->i(1), 1, 'int');
            }
            $_for0_first = 0;
            if (!($rt->binary('<=', $ky, $rt->i(1)))) {
                last;
            }
            $kx = $rt->unary('-', $rt->i(1));
            $_for1_first = 1;
            for my $_for1 (0 .. 1048575) {
                if (!$_for1_first) {
                    $kx = $rt->binary('+', $kx, $rt->i(1), 1, 'int');
                }
                $_for1_first = 0;
                if (!($rt->binary('<=', $kx, $rt->i(1)))) {
                    last;
                }
                $sampleX = $wrapCoord__int_int->($rt->binary('+', $rt->swizzle($coord, 'x'), $rt->binary('*', $kx, $offset, 1, 'int'), 1, 'int'), $rt->swizzle($dimensions, 'x'));
                $sampleY = $wrapCoord__int_int->($rt->binary('+', $rt->swizzle($coord, 'y'), $rt->binary('*', $ky, $offset, 1, 'int'), 1, 'int'), $rt->swizzle($dimensions, 'y'));
                $samples->[int($idx)] = $rt->swizzle($rt->texel_fetch($_u_valueTexture, $rt->construct(2, $sampleX, $sampleY, 'int'), $rt->i(0)), 'r');
                $idx = $rt->binary('+', $idx, $rt->i(1), 1, 'int');
            }
        }
        $gx = $rt->binary('+', $rt->binary('-', $rt->binary('+', $rt->binary('-', $rt->binary('+', $rt->unary('-', $samples->[int($rt->i(0))]), $samples->[int($rt->i(2))], 1, 'float'), $rt->binary('*', $rt->f(2), $samples->[int($rt->i(3))], 1, 'float'), 1, 'float'), $rt->binary('*', $rt->f(2), $samples->[int($rt->i(5))], 1, 'float'), 1, 'float'), $samples->[int($rt->i(6))], 1, 'float'), $samples->[int($rt->i(8))], 1, 'float');
        $gy = $rt->binary('+', $rt->binary('+', $rt->binary('+', $rt->binary('-', $rt->binary('-', $rt->unary('-', $samples->[int($rt->i(0))]), $rt->binary('*', $rt->f(2), $samples->[int($rt->i(1))], 1, 'float'), 1, 'float'), $samples->[int($rt->i(2))], 1, 'float'), $samples->[int($rt->i(6))], 1, 'float'), $rt->binary('*', $rt->f(2), $samples->[int($rt->i(7))], 1, 'float'), 1, 'float'), $samples->[int($rt->i(8))], 1, 'float');
        $magnitude = $distanceMetric__float_float_int->($gx, $gy, $metric);
        $normalized = $rt->component_wise('clamp', $rt->binary('*', $magnitude, $rt->f(4), 1, 'float'), $rt->f(0), $rt->f(1));
        @{$g->{fragColor}} = map { $rt->f32($_) } @{($rt->construct(4, $normalized, $normalized, $normalized, $rt->f(1)))};
    };
    $main__void->();
    my $_c0 = $g->{fragColor};
    $out->[0] = $rt->f32($_c0->[0]); $out->[1] = $rt->f32($_c0->[1]); $out->[2] = $rt->f32($_c0->[2]); $out->[3] = $rt->f32($_c0->[3]);
};
{ kernel => $run_pixel, uses_derivatives => 0, output_names => ['fragColor'] };
