...
my $run_pixel = sub {
    ...
    my $_u_blurRadius = exists $U->{'blurRadius'} ? $U->{'blurRadius'} : 0;
    my $_u_blurAmount = exists $U->{'blurAmount'} ? $U->{'blurAmount'} : $rt->f(0.0);
    my $_u_renderScale = exists $U->{'renderScale'} ? $U->{'renderScale'} : $rt->f(0.0);
    $g->{fragColor} = $rt->construct(4, 0.0);
    $main__void = sub {
        my ($_for0_first, $_for1_first, $blurred, $center, $coord, $dist2, $kx, $ky, $result, $samplePos, $scaledRadius, $sigma, $sigma2, $sum, $texSample, $texSize, $weight, $weightSum);
        ...
        if ((($rt->binary('<=', $scaledRadius, $rt->i(0))) || ($rt->binary('<=', $_u_blurAmount, $rt->f(0))) ? 1 : 0)) {
        ...
        for my $_for0 (0 .. 1048575) {
            ...
            for my $_for1 (0 .. 1048575) {
                ...
                @{$sum} = map { $rt->f32($_) } @{($rt->binary('+', $sum, $rt->binary('*', $rt->swizzle($texSample, 'rgb'), $weight, 3, 'float'), 3, 'float'))};
