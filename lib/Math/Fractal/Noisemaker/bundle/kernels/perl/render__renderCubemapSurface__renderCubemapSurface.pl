...
my $run_pixel = sub {
    ...
    my $U = $ctx->uniforms;
    my $g = {};
    my ($atlasTexel__ivec3_int, $sampleVolume__vec3, $intersectBox__vec3_vec3, $main__void);
    my $_retc;
    ...
    my $_u_absorption = exists $U->{'absorption'} ? $U->{'absorption'} : $rt->f(0.0);
    my $_u_emission = exists $U->{'emission'} ? $U->{'emission'} : $rt->f(0.0);
    ...
    $main__void = sub {
        ...
        $t = $rt->f(0.0);
        $t0 = $rt->f(0.0);
        if ($rt->binary('>', $rt->swizzle($tb, 'y'), $rt->f(0))) {
