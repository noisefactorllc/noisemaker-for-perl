...
my $run_pixel = sub {
    ...
    my $_u_prevFrameTex = $ctx->texture_binding('prevFrameTex');
    ...
    $main__void = sub {
        my ($globalCoord, $nd2, $scaling, $state, $texSize, $texelSize, $uv, $v0, $v00, $v01, $v1, $v10, $v11, $xAmount, $xy, $yAmount);
        ...
        @{$g->{fragColor}} = map { $rt->f32($_) } @{($rt->construct(4, $state, $state, $state, $rt->f(1)))};
    };
    $main__void->();
    my $_c0 = $g->{fragColor};
